CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS deductDiscount_input FOR MODIFY
      IMPORTING keys FOR ACTION Travel~deductDiscount_input RESULT result.
...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    DATA:
      entity           TYPE STRUCTURE FOR CREATE ZRAP100_R_TravelTP1,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap100_atrav1 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap100_dtrav1 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF ZRAP100_R_TravelTP1 IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP1 IN LOCAL MODE
      ENTITY Travel
        UPDATE SET FIELDS
...
**************************************************************************
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZRAP100_R_TravelTP1.
...
**************************************************************************
  METHOD copyTravel.
    DATA:
       travels       TYPE TABLE FOR CREATE zrap100_r_traveltp1\\travel.
    ...
    result = VALUE #( FOR travel IN travels
                      ( %tky                   = travel-%tky
                        ...
                        %action-deductDiscount = COND #( WHEN travel-OverallStatus = travel_status-open
                                                          THEN if_abap_behv=>fc-o-enabled
                                                          ELSE if_abap_behv=>fc-o-disabled   )
                        %action-deductDiscount_input = COND #( WHEN travel-OverallStatus = travel_status-open
                                                          THEN if_abap_behv=>fc-o-enabled
                                                          ELSE if_abap_behv=>fc-o-disabled   )
                     ) ).
